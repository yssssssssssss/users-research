      ...
      >
        <div>
          ...
          <Paragraph type="secondary" style={{ marginBottom: 0 }}>
            聚合当前输出的结论、证据摘要和下一步动作，适合演示、评审与案例回放。
          </Paragraph>
        </div>
        <Space wrap>
          <Button><Link to={TASK_DETAIL_OVERVIEW_PATH(currentTaskId)}>返回总览</Link></Button>
          <Button><Link to={TASK_DETAIL_EVIDENCE_PATH(currentTaskId)}>查看证据池</Link></Button>
          <Button type="primary"><Link to={TASK_DETAIL_REPORT_PATH(currentTaskId)}>进入综合报告</Link></Button>
        </Space>
      </Space>
