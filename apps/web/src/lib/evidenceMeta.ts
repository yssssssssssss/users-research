import type { EvidenceItem } from '@users-research/shared';
...
export type EvidenceAuthenticityKind =
  | 'reviewed_external'
  | 'fetched_article'
  | 'fetched_document'
  | 'search_result'
...
export const getEvidenceAuthenticityKind = (item: EvidenceItem): EvidenceAuthenticityKind => {
...
export const getEvidenceAuthenticityTag = (item: EvidenceItem): { label: string; color: string } => {
...
export const getEvidenceSourceDomain = (item: EvidenceItem): string | undefined => {
  const trace = asRecord(item.traceLocation);
