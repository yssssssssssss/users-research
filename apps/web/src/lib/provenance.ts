...
export interface ProvenanceSummary {
  ...
  personaFindingCount: number;
  fallbackWarnings: string[];
  boundaryNotes: string[];
  tags: ProvenanceTag[];
  riskLevel: 'clear' | 'mixed' | 'fallback';
}
...
export const buildBoundaryNotes = (options: {
...
export const buildProvenanceSummary = (options: {
  ...
  const acceptedRealEvidence = evidencePool.filter(
    (item) =>
  ...
  const pendingExternalEvidenceCount = evidencePool.filter(
    (item) =>
  ...
  const simulatedEvidenceCount = evidencePool.filter(
    (item) =>
      item.sourceLevel === 'simulated' ||
