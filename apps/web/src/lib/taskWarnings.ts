export interface TaskWarningGroups {
  authenticityDowngrade: string[];
  otherWarnings: string[];
}

const AUTHENTICITY_DOWNGRADE_PREFIX = '已对综合判断应用真实性降级：';

export const splitTaskWarnings = (warnings: string[] | undefined): TaskWarningGroups => {
  const groups: TaskWarningGroups = {
